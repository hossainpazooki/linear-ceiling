"""Append entry 0044 -- the E9 long half of the second model family RAN; figures only, written ONLY from an
...
Ordering guard: 0043 on the ledger, 0044 absent; the summary must PASS (it refuses on anything wrong and
...
Per entry 0043: coverage "n scored of N registered" travels with every number; the band word is computed
...
if partial:
    assert a.cutoff_reason, "the report is a partial close: --cutoff-reason is required (entry 0043 stopping rule)"
else:
...
assert rope and rope.get("recorded"), \
    ("the run's dumps carry no RoPE spec, so neither the native-window nor the frequency-identity control ran -- "
     "and on a natively long receiver those two ARE the control (entry 0043)")
